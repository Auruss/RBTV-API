 ...
 */
export function loadController(folder: string) {
    var dirs = scanDirectory("dev/src/controller/" + folder);

    dirs.forEach((dir: string) => {
        var moduleName = dir.substr(0, dir.indexOf('.'));
        var module: string = "./controller/" + folder + '/' + moduleName;
...
export function loadJobs() {
    var dirs = scanDirectory('dev/src/jobs/', 'js');

    dirs.forEach((dir: string) => {
        var jobName = dir.substr(0, dir.indexOf('.'));
        jobs.addJob(jobName);
    });
}